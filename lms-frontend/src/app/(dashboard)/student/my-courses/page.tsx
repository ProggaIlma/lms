      ...
      {enrollments.length === 0 ? (
        ...
        <div className="space-y-3">
          {enrollments.map((enrollment) => (
            <div key={enrollment.id} className="bg-white rounded-xl border border-surface-200 p-5">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-surface-900 text-sm truncate">{enrollment.course.title}</p>
                  <p className="text-xs text-surface-400 mt-0.5">
                    {enrollment.course.instructor?.name}
                    {enrollment.course.category && ` · ${enrollment.course.category.name}`}
                    {` · ${enrollment.course._count?.lessons ?? 0} lessons`}
                  </p>
                  <p className="text-xs text-surface-400 mt-0.5">Enrolled {formatDate(enrollment.enrolledAt)}</p>
                </div>
                ...
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  {enrollment.status === "ACTIVE" && (
                    <Button variant="danger" size="sm" isLoading={updatingId === enrollment.id} onClick={() => handleDrop(enrollment.id)}>
                      Drop
                    </Button>
                  )}
              ...
              <div className="mt-4">
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="text-xs font-semibold text-primary-600">{enrollment.progress ?? 0}%</span>
                </div>
                <div className="h-1.5 bg-surface-100 rounded-full overflow-hidden">
                  <div className="h-full bg-primary-600 rounded-full transition-all duration-500" style={{ width: `${enrollment.progress ?? 0}%` }} />
                </div>
              </div>
            </div>
          ))}
